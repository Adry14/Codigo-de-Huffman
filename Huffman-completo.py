...
# Variable global para almacenar los códigos de Huffman
codigos = None
...
def abrir_archivo():
    ...
    if filepath:
        ...
        print("Lista de frecuencias de caracteres:")
        print(frecuencias)
        ...
        # Guardar el texto comprimido en un archivo binario con otro nombre
        nombre_archivo_comprimido = filedialog.asksaveasfilename(defaultextension=".bin")
        with open(nombre_archivo_comprimido, 'wb') as file:
            file.write(texto_comprimido.encode())
        
        text_widget.delete('1.0', tk.END)  # Limpia el widget de texto
        # Muestra las frecuencias y el texto comprimido en la GUI
...
# Función para descomprimir el archivo comprimido seleccionado.
def descomprimir_archivo():
    global codigos
    filepath = filedialog.askopenfilename()  # Abre un cuadro de diálogo para seleccionar un archivo
    if filepath:
        with open(filepath, 'rb') as file:
            texto_comprimido = file.read().decode()
        
        texto_descomprimido = descomprimir(texto_comprimido, codigos)
        print("Texto Descomprimido:")
        print(texto_descomprimido)
        
        nombre_archivo_descomprimido = filedialog.asksaveasfilename(defaultextension=".txt")
        with open(nombre_archivo_descomprimido, 'w', encoding='utf-8') as file:
            file.write(texto_descomprimido)
        
        # Mostrar el texto descomprimido en la GUI
...
btn_examinar = tk.Button(root, text="Examinar y Comprimir", command=abrir_archivo)
btn_examinar.pack(pady=10)  # Coloca el botón en la ventana principal con un padding vertical

btn_descomprimir = tk.Button(root, text="Descomprimir", command=descomprimir_archivo, state=tk.DISABLED)
btn_descomprimir.pack(pady=10)  # Coloca el botón de descompresión en la ventana principal con un padding vertical
